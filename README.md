...
python -m venv lead\_gen



\# Activate environment (Windows)

.\\lead\_gen\\Scripts\\activate



\# Activate environment (Mac/Linux)

source lead\_gen/bin/activate
...
1\. \*\*Configure Target Parameter Profiles:\*\* Enter your localized search context query (e.g., `Pizza, Katerini` or `Burgers, Washington`) and set your target lead collection quotas directly inside the web interface.

2\. \*\*Execute the Scraper:\*\* Click the "Click me to start scraping!!!" button. This automatically initializes a safe background worker instance via `main.py`.

3\. \*\*Explore the Leads Warehouse:\*\* The dashboard automatically invalidates older state caches, pulls the live rows from `database.db`, and updates the interactive interactive lookup layout profiles on the fly.
...
| `phone\_number` | `TEXT` |  | Telephony Route Contact Value |
